...
kubectl apply -f k8s/02-customer-service.yaml
kubectl apply -f k8s/03-supplier-service.yaml
kubectl apply -f k8s/04-product-service.yaml
kubectl apply -f k8s/05-import-service.yaml
```
Hoặc gọn:
```bash
kubectl apply -f k8s/
...
kubectl logs -n qlysanbong deploy/customer-service
```

## 6. Truy cập service
```bash
minikube service customer-service -n qlysanbong
...
```bash
minikube service customer-service -n qlysanbong --url
```

NodePort đã set:
| Service           | Port nội bộ | NodePort |
|-------------------|-------------|----------|
| customer-service  | 8081        | 30081    |
...
```bash
docker build -t customer-service:latest ./customer-service
kubectl rollout restart deploy/customer-service -n qlysanbong
```
...
- 4 DB (`customer_db`, `supplier_db`, `product_db`, `import_db`) tự tạo nhờ `createDatabaseIfNotExist=true`.
